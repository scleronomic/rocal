...
from Kinematic.frames import apply_noise, trans_rotvec2frame
from Kinematic.frames_diff import frame_difference

from Justin.Calibration.calibration import (kinematic, set_bool_dict_false, hist_frame_difference,
                                            get_active_parameters, get_x_bool_dict, unwrap_x)
from Justin.Calibration.Measurements.io2 import load_q, save_m
...
def create_dummy_measurements(*, cal_rob, x_dict, q,
    ...
    if verbose > 0:
        ...
        if verbose > 1:
            figs = [hist_frame_difference(diff_trans=trans_diff0[:, i], diff_rot=rot_diff0[:, i],
...
def value_analysis_cp():
    ...
    # joint stiffnesses torso 1e-2, 1e-4, 1eâ-2
    # quer 3e-3
...
def main_create_dummy_measurements():
    ...
    dcmf = 'cc0c'
    cal_rob_ff = Justin19Calib(dcmf=dcmf, cp_loop=30, config_filter='ff', use_imu=False, add_nominal_offsets=True, fr0=False)
    cal_rob_nf = Justin19Calib(dcmf=dcmf, cp_loop=30, config_filter='nf', use_imu=False, add_nominal_offsets=True, fr0=False)
